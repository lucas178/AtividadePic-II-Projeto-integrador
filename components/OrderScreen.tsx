...
({ label, name, value, onChange, placeholder, className, type = 'text', maxLength }) => (
    <div className={className}>
        <label htmlFor={name} className="text-sm font-semibold text-slate-500 dark:text-slate-400 px-1">{label}</label>
        <input 
            id={name}
            name={name}
            type={type}
...
({ label, icon, isSelected, onClick }) => (
    <button 
        ...
        className={`w-full flex items-center p-3 rounded-lg border-2 transition-all duration-200 ${
            isSelected ? 'border-pink-500 bg-pink-50 dark:bg-pink-900/20' : 'border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700'
        }`}
    >
        <div className={`mr-3 ${isSelected ? 'text-pink-500' : 'text-slate-500 dark:text-slate-400'}`}>{icon}</div>
        <span className={`font-semibold ${isSelected ? 'text-pink-600 dark:text-pink-400' : 'text-slate-700 dark:text-slate-300'}`}>{label}</span>
    </button>
);
...
const OrderScreen: React.FC<OrderScreenProps> = ({ setScreen, cart, onFinalizeOrder }) => {
  ...
  const renderPaymentDetails = () => {
    switch (paymentMethod) {
        ...
        return (
          <div className="space-y-3 animate-fade-in">
            <div className="flex items-center space-x-2 mb-2">
                <CreditCardIcon className="h-6 w-6 text-slate-600 dark:text-slate-300" />
                <h3 className="text-md font-semibold text-slate-700 dark:text-slate-200">Dados do Cartão</h3>
            </div>
      ...
      case 'Pix':
        return (
            <div className="text-center flex flex-col items-center space-y-4 animate-fade-in p-2 bg-slate-50 dark:bg-slate-700 rounded-lg shadow-sm">
                <h3 className="font-semibold text-slate-700 dark:text-white">Pague com Pix</h3>
                <div className="bg-white p-2 border border-slate-200 dark:border-slate-600 rounded-lg shadow-inner">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300 max-w-xs">Use o app do seu banco para ler o QR Code ou copie o código abaixo.</p>
                <div className="w-full p-2 bg-slate-200 dark:bg-slate-600 rounded-lg text-xs text-slate-700 dark:text-slate-200 font-mono break-all">
                   {pixKey}
                </div>
                <button 
                    onClick={handleCopyPix}
                    className="w-full flex items-center justify-center space-x-2 bg-slate-600 dark:bg-slate-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-slate-700 transition-colors"
                >
        ...
        return (
          <div className="space-y-4 animate-fade-in">
            <div className="flex items-center space-x-2">
                <CashIcon className="h-6 w-6 text-slate-600 dark:text-slate-300" />
                <h3 className="text-md font-semibold text-slate-700 dark:text-slate-200">Pagamento na Entrega</h3>
            </div>
            <div>
                <p className="text-sm font-semibold text-slate-600 dark:text-slate-400 mb-2">Precisa de troco?</p>
                <div className="flex space-x-3">
                    <button 
                        onClick={() => {
                            setNeedsChange('no');
                        ...
                        className={`flex-1 font-bold py-2 px-4 rounded-lg border-2 transition-colors ${
                            needsChange === 'no' ? 'bg-pink-500 text-white border-pink-500' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700'
                        }`}
                    >Não</button>
                    <button
                        onClick={() => {
                        ...
                        className={`flex-1 font-bold py-2 px-4 rounded-lg border-2 transition-colors ${
                            needsChange === 'yes' ? 'bg-pink-500 text-white border-pink-500' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700'
                        }`}
                    >Sim</button>
                </div>
            </div>
    ...
    return (
        <div className="bg-slate-100 dark:bg-slate-950 h-full flex flex-col transition-colors duration-300">
            ...
            <div className="flex-grow flex flex-col justify-center items-center text-center p-6">
                <p className="text-lg text-slate-500 dark:text-slate-400 mb-4">Seu carrinho está vazio.</p>
                <button 
  ...
  return (
    <div className="bg-slate-100 dark:bg-slate-950 h-full flex flex-col transition-colors duration-300">
      ...
      <div className="p-6 space-y-4 flex-grow overflow-y-auto">
        {activeCart.map(item => (
          <div key={item.cupcake.id} className="flex items-center bg-white dark:bg-slate-800 p-3 rounded-lg shadow-md transition-colors duration-300">
            <img src={item.cupcake.image} alt={item.cupcake.name} className="w-12 h-12 rounded-md object-cover" />
            <div className="ml-4 flex-grow">
              <p className="font-semibold text-slate-700 dark:text-slate-200">{item.quantity}x {item.cupcake.name}</p>
            </div>
            <p className="font-bold text-slate-800 dark:text-white">R$ {(item.quantity * item.cupcake.price).toFixed(2)}</p>
          </div>
        ))}
        
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md space-y-2 transition-colors duration-300">
            <div className="flex justify-between items-center">
                <span className="text-md text-slate-600 dark:text-slate-400">Subtotal</span>
                <span className="text-md text-slate-700 dark:text-slate-200">R$ {subtotal.toFixed(2)}</span>
            </div>
            {discount > 0 && (
            ...
            <div className="flex justify-between items-center">
                <span className="text-md text-slate-600 dark:text-slate-400">Taxa de Entrega</span>
                <span className="text-md text-slate-700 dark:text-slate-200">R$ {DELIVERY_FEE.toFixed(2)}</span>
            </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md transition-colors duration-300">
            ...
            >
                <span className="font-semibold text-slate-700 dark:text-white">DADOS DO CLIENTE</span>
                 <div className="flex items-center space-x-2">
                    {Object.values(customer).every(field => field) && !isCustomerOpen && (
                        <CheckboxIcon className="h-6 w-6 text-green-500" />
                    )}
                    <ChevronDownIcon className={`h-6 w-6 text-slate-500 dark:text-slate-400 transition-transform duration-300 ${isCustomerOpen ? 'rotate-180' : ''}`} />
                </div>
            </button>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md transition-colors duration-300">
            ...
            >
                <span className="font-semibold text-slate-700 dark:text-white">ENDEREÇO DE ENTREGA</span>
                <div className="flex items-center space-x-2">
                    {Object.values(address).every(field => field) && !isAddressOpen && (
                        <CheckboxIcon className="h-6 w-6 text-green-500" />
                    )}
                    <ChevronDownIcon className={`h-6 w-6 text-slate-500 dark:text-slate-400 transition-transform duration-300 ${isAddressOpen ? 'rotate-180' : ''}`} />
                </div>
            </button>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md transition-colors duration-300">
            ...
            >
                <span className="font-semibold text-slate-700 dark:text-white">FORMA DE PAGAMENTO</span>
                 <div className="flex items-center space-x-2">
                    {paymentMethod && !isPaymentOpen && (
                        <CheckboxIcon className="h-6 w-6 text-green-500" />
                    )}
                    <ChevronDownIcon className={`h-6 w-6 text-slate-500 dark:text-slate-400 transition-transform duration-300 ${isPaymentOpen ? 'rotate-180' : ''}`} />
                </div>
            </button>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md transition-colors duration-300">
            <label className="text-sm font-semibold text-slate-500 dark:text-slate-400 px-1">CUPOM DE DESCONTO</label>
            <div className="flex items-center space-x-2 mt-1">
                <input 
                    ...
                    className={`w-full p-3 bg-white dark:bg-slate-800 text-slate-900 dark:text-white border rounded-lg focus:ring-2 focus:ring-pink-400 focus:border-transparent transition placeholder:text-slate-400 dark:placeholder:text-slate-500 ${couponError ? 'border-red-500 ring-red-300' : 'border-slate-300 dark:border-slate-700'}`}
                />
                <button
                  onClick={handleApplyCoupon}
                  className="bg-slate-600 dark:bg-slate-700 text-white font-bold py-3 px-5 rounded-lg hover:bg-slate-700 dark:hover:bg-slate-600 transition-colors flex-shrink-0"
                >
                  Aplicar
                </button>
            </div>
            {couponError && <p className="text-red-500 text-sm mt-2">{couponError}</p>}
            {discount > 0 && <p className="text-green-600 dark:text-green-400 text-sm mt-2">Cupom de {(discount * 100).toFixed(0)}% aplicado com sucesso!</p>}
        </div>
      </div>
      <div className="p-6 grid grid-cols-2 gap-4">
        <button 
          onClick={() => setShowCancelConfirm(true)}
          className="w-full bg-slate-300 dark:bg-slate-700 text-slate-700 dark:text-white font-bold py-3 px-4 rounded-lg hover:bg-slate-400 dark:hover:bg-slate-600 transition-colors"
        >
          CANCELAR
        </button>
        <button 
      ...
      {showCancelConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 animate-fade-in">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg max-w-sm w-full mx-4 transition-colors duration-300">
                <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-4">Confirmar Cancelamento</h3>
                <p className="text-slate-600 dark:text-slate-300 mb-6">Tem certeza que deseja cancelar este pedido?</p>
                <div className="flex justify-end space-x-4">
                    <button
                        onClick={() => setShowCancelConfirm(false)}
                        className="bg-slate-300 dark:bg-slate-700 text-slate-700 dark:text-white font-bold py-2 px-4 rounded-lg hover:bg-slate-400 dark:hover:bg-slate-600 transition-colors"
                    >
                        Não
                    </button>
                    <button
